...
const Landing = () => {
  ...
  let textLeft = textForLeft.split("").map((el, id) => {
    return (
  ...
  let textRight = textForRight.split("").map((el, id) => {
    return (
      <span key={id} className="letter">
        {el}
      </span>
  ...
  let textCenter = textForCenter.split("").map((el, id) => {
    return (
      <span key={id} className={`letter letter-row`}>
        {el}
      </span>
    );
  });
  ...
  return (
    <div className="Landing">
      <div className="App-line">
        <h1 className="App-row">{textLeft}</h1>
        ...
        <h1 className="App-row">{textRight}</h1>
      </div>
    </div>
  );
};
